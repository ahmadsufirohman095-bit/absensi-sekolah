...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between transition-all duration-300 hover:shadow-md hover:border-indigo-500/50">
    <div class="flex items-center">
        <div class="mr-4">
            @if($jadwal->absensi_masuk_tepat_waktu)
            ...
            @elseif($jadwal->absensi_masuk_terlambat)
        ...
        <div>
            <p class="font-bold text-gray-800 dark:text-gray-100">{{ $jadwal->mataPelajaran->nama_mapel }}</p>
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $jadwal->kelas->nama_kelas }}</p>
    ...
    <div>
        @if(!$jadwal->absensi_masuk)
            <a href="{{ route('scan.index', ['jadwal_id' => $jadwal->id]) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-25 transition ease-in-out duration-150">
                Mulai Absensi
            </a>
        @else
