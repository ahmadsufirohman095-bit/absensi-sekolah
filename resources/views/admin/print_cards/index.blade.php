<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg shadow-sm" x-data="tomSelectManager">
                        ...
                        <div>
                            <form action="{{ route('print-cards.index') }}" method="GET" id="filterForm">
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end mb-4">
                                    <div>
                                        ...
                                        <select name="role" id="role" class="block w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                            ...
                                            <option value="other" {{ request('role') == 'other' ? 'selected' : '' }}>Lainnya</option>
                                        </select>
                                    </div>
                                    <div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="text" name="search" id="search" class="block w-full pl-10 pr-3 py-2 rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Cari..." value="{{ request('search') }}">
                                        </div>
                                    </div>
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-end mb-4">
                                    <div class="md:col-span-1">
                                        ...
                                        <select name="config_id" id="config_id" class="block w-full rounded-md shadow-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                            @forelse($configs as $config)
                                                <option value="{{ $config->id }}" {{ $selectedConfigId == $config->id ? 'selected' : '' }}>{{ $config->name }} {{ $config->is_default ? '(Default)' : '' }}</option>
                                            @empty
                                                <option value="">Tidak ada konfigurasi tersedia</option>
                                            @endforelse
                                        </select>
                                        @if($configs->isEmpty())
                                            <p class="mt-2 text-red-500 text-sm dark:text-red-400">Belum ada konfigurasi kartu. Silakan buat di <a href="{{ route('absensi.cards.customize') }}" class="text-blue-500 dark:text-blue-400 hover:underline">halaman kustomisasi</a>.</p>
                                        @endif
                                    </div>
                                    <div class="flex justify-end gap-2 md:col-span-1">
                                        ...
                                        <a href="{{ route('print-cards.index') }}" class="px-4 py-2 bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">Reset Filter</a>
                                    </div>
                                </div>
                            </form>
                        </div>
                                ...
                                <table class="table table-bordered table-hover text-gray-900 dark:text-gray-100" id="usersTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="dark:bg-gray-800 dark:hover:bg-gray-600">
                                            <td class="p-2"><input type="checkbox" class="user-checkbox" value="{{ $user->id }}"></td>
                                            <td class="p-2">{{ $user->name }}</td>
                                            <td class="p-2">
                                                {{ ucfirst($user->role) }}
                                                @if ($user->role == 'other' && $user->custom_role)
                                                    (<span class="text-purple-600 dark:text-purple-300">{{ $user->custom_role }}</span>)
                                                @endif
                                            </td>
                                            <td class="p-2">{{ $user->identifier }}</td>
                                            <td class="p-2">{{ $user->email }}</td>
                                            <td class="p-2">
                                                <span class="badge badge-{{ $user->is_active ? 'success' : 'danger' }} px-2 py-1 rounded-full text-xs {{ $user->is_active ? 'bg-green-500 text-white' : 'bg-red-500 text-white' }}">
                                                    {{ $user->is_active ? 'Aktif' : 'Nonaktif' }}
                                                </span>
                                            </td>
                                            <td class="p-2">
                                                <a href="javascript:void(0)" class="btn btn-sm btn-info print-single-card bg-blue-500 hover:bg-blue-600 text-white font-bold py-1 px-2 rounded text-xs" data-user-id="{{ $user->id }}" title="Cetak Kartu Individual">
    ...
    <form id="printForm" action="{{ route('print-cards.generate') }}" method="POST" target="_blank">
        @csrf
        {{-- <input type="hidden" name="user_ids[]" id="hiddenUserIds"> --}} <!-- Dihapus karena dibuat dinamis -->
