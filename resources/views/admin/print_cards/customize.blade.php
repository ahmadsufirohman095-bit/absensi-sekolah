                            ...
                            <form @submit.prevent="saveChanges()" class="space-y-6">
                                ...
                                <div class="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                                        <div><x-input-label for="bg_color" :value="__('Latar Kartu')" /><input type="color" id="bg_color" x-model="selectedConfig.config_json.theme.background_color" class="w-full h-10 p-1 bg-white dark:bg-gray-800 border rounded-lg cursor-pointer"></div>
                                        <div><x-input-label for="header_bg_color" :value="__('Latar Header')" /><input type="color" id="header_bg_color" x-model="selectedConfig.config_json.theme.header_background_color" class="w-full h-10 p-1 bg-white dark:bg-gray-800 border rounded-lg cursor-pointer"></div>
                                        <div><x-input-label for="text_header" :value="__('Teks Header')" /><input type="color" id="text_header" x-model="selectedConfig.config_json.theme.text_color_header" class="w-full h-10 p-1 bg-white dark:bg-gray-800 border rounded-lg cursor-pointer"></div>
                                        <div><x-input-label for="text_body" :value="__('Teks Body')" /><input type="color" id="text_body" x-model="selectedConfig.config_json.theme.text_color_body" class="w-full h-10 p-1 bg-white dark:bg-gray-800 border rounded-lg cursor-pointer"></div>
                                    </div>
                                    <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <x-input-label for="bg_opacity">Transparansi Latar Kartu (<span x-text="selectedConfig.config_json.theme.background_opacity"></span>)</x-input-label>
                                        ...
                                        <div>
                                            <x-input-label for="header_opacity">Transparansi Latar Header (<span x-text="selectedConfig.config_json.theme.header_background_opacity"></span>)</x-input-label>
                                ...
                                <div class="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <x-input-label for="header_title" :value="__('Judul Header')" />
                                            <x-text-input id="header_title" class="block mt-1 w-full" type="text" x-model="selectedConfig.config_json.header_title" />
                                        </div>
                                        <div>
                                            <x-input-label for="school_name" :value="__('Nama Sekolah')" />
                                            <x-text-input id="school_name" class="block mt-1 w-full" type="text" x-model="selectedConfig.config_json.school_name" />
                                        </div>
                                ...
                                <div class="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                    ...
                                    <!-- Logo -->
                                    <div class="mb-4">
                                        <x-input-label for="logo_upload" :value="__('Logo Sekolah')" />
                                    ...
                                    <!-- Watermark -->
                                    <div class="mb-4">
                                        <x-input-label for="watermark_upload" :value="__('Gambar Watermark')" />
                                        <div class="flex items-center space-x-2 mt-1">
                                    ...
                                    <div class="mt-4"><x-input-label for="watermark_opacity">Transparansi Watermark (<span x-text="selectedConfig.config_json.assets.watermark_opacity"></span>)</x-input-label><input type="range" id="watermark_opacity" min="0" max="1" step="0.05" x-model.number="selectedConfig.config_json.assets.watermark_opacity" class="w-full"></div>
                                    <div class="mt-4">
                                        ...
                                        <div class="card-preview rounded-lg" :class="selectedConfig.card_orientation" :style="`background-color: ${hexToRgba(selectedConfig.config_json.theme.background_color, selectedConfig.config_json.theme.background_opacity)};`">
                                            ...
                                            <div class="card-header-preview" :style="`background-color: ${hexToRgba(selectedConfig.config_json.theme.header_background_color, selectedConfig.config_json.theme.header_background_opacity)}; color: ${selectedConfig.config_json.theme.text_color_header}; padding-left: ${selectedConfig.config_json.header_padding_x}px; padding-right: ${selectedConfig.config_json.header_padding_x}px;`">
                                                ...
                                                <div>
                                                    ...
                                                    <p style="font-size: 10px;" x-text="selectedConfig.config_json.school_name || '{{ setting('nama_sekolah', 'Nama Sekolah') }}'"></p>
                                                </div>
                                            </div>
        ...
        function cardCustomizer() {
            return {
                ...
                async uploadAsset(event, assetType) {
                    ...
                    try {
                        const response = await fetch('{{ route("card-assets.store") }}', {
                            method: 'POST',
                            headers: {
                ...
                async deleteAsset(assetType) {
                    ...
                    try {
                        const response = await fetch('{{ route("card-assets.destroy") }}', {
                            method: 'DELETE',
                            headers: {
                ...
                async saveChanges() {
                    ...
                    const url = isNew ? '{{ url("/print-card-configs") }}' : `{{ url("/print-card-configs") }}/${this.selectedConfig.id}`;
                ...
                async deleteConfig(id) {
                    ...
                    try {
                        const response = await fetch(`{{ url("/print-card-configs") }}/${id}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Accept': 'application/json',
